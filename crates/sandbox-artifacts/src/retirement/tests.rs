...
use super::*;
use crate::tests::{TESTS, memory, owner, plan};
use object_store::ObjectStoreExt;

struct NoDelete;
impl VersionDelete for NoDelete {
    fn delete<'a>(&'a self, _: &'a OutputPlan, _: &'a str) -> BoxFuture<'a, Result<(), Error>> {
        Box::pin(async { panic!("unversioned retirement must never delete a key") })
    }
}
fn retirer(store: &ArtifactStore) -> ArtifactRetirer {
    ArtifactRetirer {
        store: store.clone(),
...
async fn retirement_checks_owner_selection_retention_and_capacity_before_mutation() {
    ...
    drop(permit);
    assert_eq!(
        store.read(&r, &p.owner, 1000, 0, 3).await.unwrap().bytes,
        b"abc"
    );
    assert_eq!(format!("{cleaner:?}"), "ArtifactRetirer { .. }");
}
...
struct LoseDeleteAck {
    inner: Arc<dyn VersionDelete>,
}
impl VersionDelete for LoseDeleteAck {
    fn delete<'a>(
        &'a self,
        plan: &'a OutputPlan,
        version: &'a str,
...
struct RefuseDelete;
impl VersionDelete for RefuseDelete {
    fn delete<'a>(&'a self, _: &'a OutputPlan, _: &'a str) -> BoxFuture<'a, Result<(), Error>> {
        Box::pin(async { Err(Error::Unavailable) })
    }
}
struct IgnoreDelete;
impl VersionDelete for IgnoreDelete {
    fn delete<'a>(&'a self, _: &'a OutputPlan, _: &'a str) -> BoxFuture<'a, Result<(), Error>> {
        Box::pin(async { Ok(()) })
    }
}
...
async fn output_minio_retirement_reclaims_exact_versions_and_recovers_interruptions() {
    let _test = TESTS.lock().await;
    for versioned in [false, true] {
        let store = config(versioned).build().unwrap();
        let cleaner = config(versioned).build_retirer().unwrap();
        for bytes in [&b"\x00\xffprivate-versioned-output"[..], &b""[..]] {
            let p = plan(bytes);
            let r = store.upload(&p, &p.owner, 1000, bytes).await.unwrap();
            assert_eq!(r.object_version.is_some(), versioned);
            if versioned {
                let interrupted = ArtifactRetirer {
...
async fn output_minio_retirement_checks_old_version_bytes_before_deletion() {
    let _test = TESTS.lock().await;
    let store = config(true).build().unwrap();
    let cleaner = config(true).build_retirer().unwrap();
    let p = plan(b"abc");
    let path = Path::from(p.object_key().unwrap());
...
impl VersionDelete for PausedDelete {
    fn delete<'a>(&'a self, _: &'a OutputPlan, _: &'a str) -> BoxFuture<'a, Result<(), Error>> {
        Box::pin(async move {
...
#[tokio::test]
#[ignore = "requires private versioned MinIO bucket"]
async fn output_minio_retirement_survives_cancelled_worker_and_concurrent_recovery() {
    let _test = TESTS.lock().await;
    let store = config(true).build().unwrap();
    let cleaner = config(true).build_retirer().unwrap();

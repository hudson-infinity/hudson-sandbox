...
async fn scan_requires_current_registered_physical_host_and_retries_durably(pool: PgPool) {
    ...
    assert!(
        f.store
            .next_allocation_retirement(f.host, 1)
            .await
            .unwrap()
            .is_none()
    );
    ...
    assert!(
        f.store
            .next_allocation_retirement(f.host, 2)
            .await
            .unwrap()
            .is_none()
    );
...
async fn concurrent_scanners_select_one_owner_once_and_skip_delivery_leases(pool: PgPool) {
    ...
    sqlx::query("UPDATE allocations SET retirement_scan_at=NULL")
        .execute(&pool)
        .await
        .unwrap();
    assert!(
...
async fn unfinished_candidates_do_not_starve_later_allocations(pool: PgPool) {
    ...
    sqlx::query(
        "UPDATE hosts SET last_allocation_serial=34,registered_allocation_serial=34 WHERE id=$1",
    )
    .bind(first.host.uuid())
    .execute(&pool)
    .await
    ...
    for _ in 0..34 {
        let candidate = first
            .store
            .next_allocation_retirement(first.host, 1)
            .await
            .unwrap()
            .unwrap();
        assert!(
            expected.remove(&candidate.allocation),
            "candidate was repeated before all owners received a turn"
        );
    }
...
#[sqlx::test(migrator = "sandbox_store::MIGRATOR")]
async fn malformed_intent_is_deferred_before_parsing_and_never_returns_authority(pool: PgPool) {
    let f = Fixture::new(&pool).await;
    released(&f).await;
    prepare(&f).await.unwrap().unwrap();
    expire(&f).await;
    sqlx::query(
        "UPDATE allocation_retirements SET intent=intent || '{\"unexpected\":true}'::jsonb",
    )
    .execute(&pool)
    .await
    .unwrap();
    assert!(matches!(
        f.store.next_allocation_retirement(f.host, 1).await,
        Err(Error::Evidence)
    ));
    assert!(
        f.store
    ...
    let preserved: bool = sqlx::query_scalar("SELECT intent->>'unexpected'='true' AND metadata_completion IS NULL AND forget_completion IS NULL FROM allocation_retirements")
        .fetch_one(&pool).await.unwrap();
    assert!(preserved);
}

#[sqlx::test(migrator = "sandbox_store::MIGRATOR")]
async fn forgetting_claims_are_deferred_and_completed_owners_are_not_redispatched(pool: PgPool) {
    use sandbox_protocol::supervisor::{
        AllocationForgetObservation, AllocationForgetRequest, AllocationForgetState,
    };
    let f = Fixture::new(&pool).await;
    released(&f).await;
    let metadata = prepare(&f).await.unwrap().unwrap();
    f.store
        .complete_allocation_retirement(&metadata, &super::completion::observation(&metadata))
        .await
        .unwrap();
    let request = f
        .store
    ...
    let observed = AllocationForgetObservation {
        request: Some(AllocationForgetRequest {
            request_json: request.encode().unwrap(),
        }),
        state: AllocationForgetState::Forgotten as i32,
    ...
    assert_eq!(
        f.store
            .allocation_forgetting_completion(&metadata.intent)
            .await
            .unwrap(),
        Some(completed)
    );

...
The guardian uses host `CLOCK_BOOTTIME` for an allocation deadline, initially derived from a bounded wall-clock lease of at most 300 seconds. A separate watchdog thread checks it independently of control handlers and journal locks. Deadline expiry terminates namespace PID 1; Linux kills its namespace descendants. Killing the outer supervisor wrapper cannot disable this watchdog. Killing the guardian likewise kills the VM descendants through namespace teardown.

...
Status: database preparation, host fencing and recoverable guardian metadata deletion are implemented components. Database completion and the bounded forgetting store/RPC handoff are implemented. An opt-in controller worker now drives the handoff. Sustained end-to-end reuse remains incomplete. This document extends [history reclamation](history-reclamation.md) for the remaining host/guardian lifecycle work in [issue #79](https://github.com/hudson-infinity/hudson-sandbox/issues/79). It does not authorize manual deletion or claim a passing release gate.
...
The retained freeze rejects further command/file IDs under the allocation lock and stops new released-history claims. Original project-lifetime operation keys, digests and retained outcomes are untouched. Database rows remain retained; this component neither refunds host receipt capacity nor deletes anything. It does not inspect host-local download pins or physical files: those remain mandatory host-side completion checks. The acknowledgement and forgetting store/RPC handoffs below are implemented. The opt-in automatic controller worker discovers eligible allocations and drives these handoffs.
...
This RPC retains its original host record, manifest, history floors and completion plan. The root entry stays fenced; it is not marked complete or forgotten. The database can retain completion through the store entry point below. The forgetting RPC below removes these protocol records and recovers their host admission slots. The opt-in controller worker below dispatches and reconciles these handoffs. Host metadata retirement alone is not a release or distribution gate.
...
The [database tests](../crates/sandbox-store/tests/support/allocation_forgetting.rs) cover issuance, independent claims, concurrent acknowledgement, lock-wait expiry, epoch changes, changed scope/evidence and upgrade preservation. The [host tests](../crates/sandbox-supervisor/tests/support/forgetting.rs) cover actual failed writes, restart windows, changed scope, reappeared resources, late callers and a real execution/retirement/new-admission cycle. The focused crash fixtures supply synthetic database bindings. The extended real API/controller lifecycle test also runs database-issued metadata and forgetting claims through the authenticated RPC, loses the host acknowledgement, consumes retired denial using retained database proof and verifies unchanged original operation rows after another restart. The controller worker below now drives fresh retirement and lost-reply recovery. More than 1,024 real execution cycles with an older live allocation are still required.

## Automatic controller worker

Enable `sandbox-controller --retire-history --retire-allocations` on a registered physical host. The allocation worker runs independently of operation dispatch, history retirement and output publication, taking one candidate per tick with a 500 ms interval. `--once` also runs one allocation tick after the four history turns. The CLI rejects combining allocation retirement with `--allow-simulated`; the library worker returns idle in simulation mode. Legacy hosts without launch authority are excluded. Output/source consumer retirement remains a prerequisite and must be configured separately where needed.

[Migration 0022](../migrations/0022_allocation_retirement_scan.sql) adds a durable scan timestamp and an index for released allocations. [Candidate selection](../crates/sandbox-store/src/allocation_retirement/scheduling.rs) requires the configured current host epoch and registered physical permit, excludes completed forgetting and live delivery claims, and uses `FOR UPDATE SKIP LOCKED`. It commits a five-second scan cooldown before parsing retained intent, so ineligible or malformed early candidates cannot indefinitely starve later owners. The query has a five-second statement timeout. This is scheduling metadata, not a cleanup claim or evidence; it never rewrites original operation results.

The [worker](../crates/sandbox-controller/src/retirement.rs) checks historical database completion before dispatch, then uses independent 120-second database claims for metadata retirement and forgetting, with a 30-second timeout per RPC. Original release and consumer closure are revalidated by preparation and completion. A lost RPC reply leaves its delivery lease intact; after expiry, a later tick obtains a fresh claim with the same frozen intent. An uncertain database acknowledgement is resolved through historical completion before any further RPC is considered. The worker consumes `Retired` only through the forgetting completion method, which independently requires retained exact metadata proof.

Completed allocations are excluded from future candidate selection. Ordinary allocation maintenance already requires running, unreleased allocations; released-domain history claims exclude every frozen retirement. Neither recovery path translates missing records or retired denial into a release proof. Original idempotency keys, digests and known operation results remain retained.

Database scheduling tests cover concurrent selection, registration/epoch checks, durable cooldown, active claims, malformed intent, completed records and progress beyond 32 unfinished candidates. Controlled real-host tests cover worker-driven fresh retirement and recovery after a lost forgetting reply, plus restart and unchanged original operation results. These are component checks; the sustained reuse and supported-host release gates below remain open.

...
Authentication is required in every environment: local development, self-hosted installations, and Hudson deployments. The [auth design](auth-design.md) defines Project access, Admin access, and browser sessions. Backend requests use the appropriate bearer credential; UI requests use a session derived from a validated credential. Both pass server-side authorization. There is no unauthenticated mode, bypass header, or configuration switch. Missing or invalid credentials and unavailable authentication storage fail closed.
...
Use opaque tokens with a cryptographically random 256-bit secret. Store only token hashes and lifecycle metadata on the project, never the raw token. Support expiry, revocation, and two active tokens per project so rotation can overlap. Provision project tokens through the authenticated Admin API/UI or local admin tooling. The management UI has Project access and Admin access login; there is no OAuth flow, JWT requirement, user directory, or custom-role editor. See [token storage](data-models.md#1-projects--ownership-and-limits) for the small record inside `projects`.

Local contributors and self-hosters use local admin setup to provision the first separate admin credential, then create projects and project tokens through authenticated admin tooling or the Admin UI. Applications keep project tokens in backend secret configuration. Setup runs under the installation administrator's authority, not through an unprotected public bootstrap endpoint. It requires no Hudson account or validation call to Hudson. The setup tooling remains to be implemented.

The project token authorizes that project's sandbox API operations. It does not grant host administration. Keep automation tokens in the trusted client backend. The management login form may submit a credential once over HTTPS and clears it after session creation; never persist it in browser storage, frontend configuration, URLs, logs, guest memory, or snapshots. Internal API/controller-to-supervisor calls use separate operator-managed service credentials over authenticated TLS, bound to the intended service/host; project tokens are never forwarded to the supervisor or guest.
...
Reauthorize every connection and recheck token/project state at most every 30 seconds while connected; fail closed on a failed check and close at token expiry. Pause closes the stream with its last cursor; resume requires a fresh authorized connection to the new allocation. The same-origin management UI streams using its validated session cookie with Origin and scope checks; logout/expiry/revocation closes it within the same recheck bound. Separate credentials for embedded third-party browser streams remain deferred. See [UI streaming](auth-design.md#live-output-and-revocation).

## Management UI

The API service also serves the Sandbox Management UI and same-origin session endpoints. Project access exposes one project's sandboxes, operations, snapshots, and usage. Admin access adds projects, quotas, project-token management, hosts, and audit. Backend policy checks every action; UI visibility is not authorization. All lifecycle actions use the existing controller, and guest content never runs on the management origin.

PostgreSQL retains the six sandbox resource models plus supporting `ui_sessions` and `audit_events` tables. Admin credentials are maintained separately in administrator-controlled deployment configuration. The [auth design](auth-design.md) is the authority for these records and routes.

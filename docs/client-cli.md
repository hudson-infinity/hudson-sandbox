...
Status: the reusable [Rust client](../crates/sandbox-client/src/lib.rs) and `hudson-sandbox` CLI cover the 14 implemented Project API operations. They use HTTPS only and have no database, supervisor or Firecracker authority. Python/TypeScript clients, package publication, signed installers and supported-host release gates remain unfinished. This is a development client for an existing installation, not a production deployment quickstart.

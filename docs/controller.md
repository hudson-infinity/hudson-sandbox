# Create controller and reconciliation

Status: a single-host create controller is implemented and tested with PostgreSQL, the authenticated HTTP router, and real loopback gRPC/mTLS to the fake supervisor. It does not run customer code. Execute, files, outputs, destroy, allocation lease renewal, and expired-allocation reconciliation still need implementation.
...
Each tick claims at most one create with a 30-second controller lease, reserves capacity, and examines persisted dispatch history. [Dispatch storage](../crates/sandbox-store/src/dispatch.rs) locks the operation, project, sandbox, and host in the same order as reservation. It constructs ownership from persisted rows, not caller-supplied copies of project or sandbox fields.
...
The crash boundary after intent but before sending remains deliberately unresolved when the supervisor reports absence. The current loop preserves that uncertainty; a future stop/fencing and destroy path must resolve it. It cannot infer that an absent in-memory record proves there was no external action in a different incarnation.
...
The image allowlist is static process configuration and checked at dispatch. Updating it currently requires restarting the controller. API admission still validates digest shape rather than this operator allowlist, so an unknown image can be admitted and later fail without being dispatched. Image manifest verification, compatibility checks, and early API policy validation remain necessary work.

...
Private `output_cleanup` rows are an auxiliary inventory for expired operation output, not a public artifact model. [Migration 0007](../migrations/0007_output_cleanup.sql) records a unique operation foreign key, claim revision/lease, retry time, immutable prepared manifest, earliest eligibility and creation time. Preparation freezes publication while preserving the exact ticket, plans and selected references. A manifest without references may describe orphan uploads; it is not proof that storage is empty. [Cleanup inventory](output-storage.md#cleanup-inventory) documents the transactional contract. The separate storage retirement primitive is implemented; scheduling it, persisting its receipts and compacting history remain unfinished. No cleanup-completed status is exposed.

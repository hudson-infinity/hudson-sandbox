...
These are host-to-guest methods. [Controller-to-supervisor upload fencing](file-transfer.md#supervisor-upload-ownership-and-recovery) is implemented; supervisor downloads and authenticated public upload/download routes remain under [issue #62](https://github.com/hudson-infinity/hudson-sandbox/issues/62). A working private guest client is not a public file API.

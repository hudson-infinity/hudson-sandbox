...
Status: initial design for implementation. Examples describe the intended contract; no API, schema migration, or ID library is implemented yet. This extends [the implementation design](implementation.md), which defines the stack and lifecycle.
...
| --- | --- | --- |
| Workspace | `wsp_<uuidv7>` | Stable tenant boundary for ownership, quotas, and authentication |
...
| Image version | `img_<uuidv7>` | Immutable runnable template version, bound to a content digest and compatibility metadata |
| Artifact | `art_<uuidv7>` | A stored output, log, or workspace export with ownership, digest, size, and retention |
| Host | `hst_<uuidv7>` | Internal registered machine identity; replacement/reprovisioning gets a new ID |

Most clients need only workspace, sandbox, operation, snapshot, image, and artifact IDs. Allocation and host identities are internal/admin details. Host addresses are not exposed in ordinary sandbox responses.
...
- A controller attempt is `(operation_id, attempt_number)`, with a positive incrementing number. It does not need another globally unique ID.
...
Provision a local workspace identity in the sandbox service. Map authenticated callers to it. An optional external reference can associate it with a Hudson workspace without coupling the ID format or requiring the harness to exist. Deleting a workspace revokes its callers and never permits reuse of its ID.
...
Deduplicate on `(workspace_id, idempotency_key)`, across mutating routes. A key represents exactly one logical request in that workspace. Store a versioned request digest covering method, canonical resource target, and the validated payload. Object field ordering is normalized; array order and command strings remain significant. Resolve defaults consistently, record effective limits/image versions separately, and preserve the normalization version so an API upgrade cannot reinterpret an old retry.

Admission is transactional:

1. Authenticate and resolve the workspace; check current access to the target or existing receipt.
2. Look up or attempt to insert the unique retry-key record. Concurrent duplicates serialize through the database uniqueness constraint.
...
Result bodies may expire, but compact retry-key tombstones retain the request digest and original operation ID for the workspace's lifetime. An identical key whose response has expired returns `410 Gone` and the original operation identity, with no execution. Reject key reuse with changed content even after result expiry. On workspace deletion, revoke access before purging its records; the workspace ID cannot be re-created. This deliberately trades small metadata retention for predictable retry behavior and must be included in storage planning.
...
The endpoints and field names below are the proposed first API shape. Full IDs are shown so the examples can be checked for format consistency. Authentication resolves the workspace; credentials are omitted.
...
{
  "image_id": "img_01996110-7c00-7000-8000-000000000010",
...
The following describes the logical schema, not executable migration SQL. All tenant-owned references include workspace ownership. Use composite foreign keys such as `(workspace_id, sandbox_id)` and matching unique constraints, even though resource UUIDs are globally generated. This prevents accidentally linking one workspace's snapshot or operation to another's sandbox.

| Table | Key fields and role |
| --- | --- |
| `workspaces` | `id`, lifecycle state, limits, optional external mapping |
| `sandboxes` | `id`, `workspace_id`, `image_id`, name/labels, desired/observed state, `state_revision`, `generation`, current allocation, current pause snapshot, active transition operation, deadlines, `destroyed_at` |
| `operations` | `id`, workspace, sandbox, kind, target operation for cancellation, digest/version, normalized payload, status/phase, claim revision/lease, deadline, pinned input references, result/error references |
| `operation_attempts` | `(operation_id, attempt_number)`, workspace, allocation, claim revision, dispatch/acknowledgement/finish times, outcome |
| `request_keys` | Unique workspace/key digest, request digest/version, operation ID, response expiry; compact tombstone after result retention |
| `allocations` | `id`, workspace, sandbox, host, supervisor epoch, generation, state, lease, reserved resources, release/fencing evidence |
| `snapshots` | `id`, workspace, sandbox, source allocation, pause operation, publication state, immutable manifest/digest, compatibility fields, retention |
| `image_versions` | `id`, workspace, immutable manifest digest, guest kernel/agent compatibility, publication state |
| `artifacts` | `id`, workspace, sandbox, producer operation, kind, object key/version, digest, size, retention, deletion state |
| `hosts` | `id`, supervisor epoch, architecture/compatibility, capacity, readiness/drain state, last observation |

The first version uses workspace-owned image versions; shared image catalogs need an explicit access model later. Create resolves an authorized, published image ID to its immutable manifest digest and pins that digest in the operation. Host identity is internal and not tenant-owned. A database-issued supervisor epoch increases at each supervisor registration after restart, forcing reconciliation before it can renew old ownership. This epoch is separate from the machine's OS boot ID and is not evidence by itself that an old VM has stopped.

Enforce these invariants:

- A sandbox belongs to one workspace for its lifetime. All operations and artifacts retain that ownership.
...
Store raw snapshot components under one snapshot, without allocating a separate public artifact ID for every memory/disk blob. Artifact IDs are for customer-visible outputs, logs, and exports. The full prefixed IDs below are represented by placeholders for readability.

```text
workspaces/{workspace_id}/sandboxes/{sandbox_id}/
...
The prefix is organization, not security. Resolve artifact/snapshot IDs through authorized metadata, keep buckets private, and issue only short-lived scoped transfer access where needed. Never accept caller-supplied bucket names or arbitrary object keys as trusted references.

## 9. Errors, retention, and implementation checks

Return `404` for inaccessible tenant resources without revealing whether another workspace owns them. An authorized caller may inspect a destroyed sandbox tombstone, but new execute/resume operations return `410 Gone`. Expired snapshots cannot be resumed. Destroy permanently prevents resume even when policy retains its snapshot bytes for a bounded period.
...
7. Database references and API reads prevent cross-workspace access, including snapshot and artifact paths.

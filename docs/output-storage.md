...
Output and file-source upload each issue one conditional create-only PUT. If that call returns a transport/provider error, the client performs one read within the existing transfer deadline and semaphore permit. Success requires complete bytes, digest and bound metadata verification. A conflicting current object returns `Conflict`; missing or unreadable data after an uncertain PUT remains `Unavailable`, because the original write might still commit. No extra PUT is issued and no reservation is refunded by this reconciliation. If the overall transfer deadline expires, the outcome remains uncertain for later reconciliation with the original persisted plan.

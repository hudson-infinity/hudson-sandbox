...
The optional `--response-retention-seconds` policy assigns terminal operation response deadlines in bounded batches before output discovery. It is disabled by default, applies to existing completed work when enabled, and never rewrites assigned deadlines or immutable output plans. [Operation retention](operation-retention.md) owns its bounds, retry/list behavior and remaining compaction work.

The separate `--compact-payloads` flag attempts one eligible body-compaction transaction per tick. It is also disabled by default. Original requests/results can be removed after verified retirement, or pending publication can be fenced when no ticket was ever issued. Command summaries preserve subsequent evidence verification and exact cleanup acknowledgement retries. [Payload compaction](operation-retention.md#payload-compaction) owns eligibility and retained-evidence requirements; host/guest journals are unaffected.
...
`expires_unix_ms` stops reads; `delete_after_unix_ms` is an earliest eligibility timestamp, not a deletion receipt. After retirement, opt-in payload compaction can remove the original request/result bodies. Execution receipts, publication history and cleanup manifests/receipts remain; archiving those histories into smaller records is unfinished. Preserve retry keys, request digests, outcomes and reconciliation evidence, and never make an old command runnable again. Destroy remains independent of publication and retirement. The API exposes missing/expired history and reconnect semantics.
...
The cleanup executable can also opt into [file source cleanup](file-transfer.md#source-cleanup-worker) with `--file-source-config`. Output and file sources use distinct workers, claims, manifests and credentials; configuring one does not grant cleanup authority for the other. Source-only runs do not require output credentials. Supplying both configurations runs one tick of each concurrently, including with `--once`.

## Uncertain upload acknowledgements

Output and file-source upload each issue one conditional create-only PUT, then confirm the current object through a read even when the PUT response is uncertain. If that read is unavailable, the client retries the read once within the original transfer deadline and semaphore permit. Both attempts verify complete bytes, digest and bound metadata from scratch; partial response bytes are discarded. Missing and conflicting read results are not retried. A conflicting current object returns `Conflict`; missing or persistently unreadable data after an uncertain PUT remains `Unavailable`, because the original write might still commit. No extra PUT is issued and no reservation is refunded by this reconciliation. If the overall transfer deadline expires, the outcome remains uncertain for later reconciliation with the original persisted plan.

The fault-injection tests cover committed writes with lost replies, uncommitted/missing writes, failed reads and conflicting objects for both output and file sources. They assert exactly one PUT and one GET. These tests establish the recovery behavior; they do not identify the underlying cause of every provider transport failure.

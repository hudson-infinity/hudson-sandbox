...
Status: the root-operated `sandbox-host` service connects authenticated lifecycle RPCs to the real [allocation guardian](allocation-guardian.md). It supports create, inspection, stop, renewal, lease inspection and public command execution/status. Retained output, live streaming and [command cancellation](command-cancellation.md) have authenticated API routes; file transfer remains unfinished. This is a development component with controlled nested aarch64 evidence, not a production or x86_64 release.
